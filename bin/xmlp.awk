...
# 
# Author:       Steve Coile <sco...@csc.com> 
# 
...
# (XML does not allow overlapping tags). 
    if ( type[idx] == "begin" ) { 
        ...
        if ( tag != tstack[sptr] ) { 
            type[idx] = "error"; 
            item[idx] = "line " lineno ": " tag ": unexpected close tag, expecting " tstack[sptr]; 
            exit( 1 ); 
        }; 
        delete tstack[sptr]; 
        sptr -= 1; 
...
# Validate the attribute name.  If invalid, indicate so and exit. 
        if ( attrib !~ /^[A-Za-z][-+_0-9A-Za-z]*$/ ) 
            { 
                type[idx] = "error"; 
                item[idx] = "line " lineno ": " attrib ": invalid attribute name";
                exit( 1 ); 
            } 
        sub( /^[^=]*/, "", $0 ); 
...
# invalid XML. 
    for ( n = sptr; n; n -= 1 ) { 
        idx += 1; 
        type[idx] = "error"; 
        item[idx] = "line " lstack[n] ": " tstack[n] ": unclosed tag"; 
    }; 
} 
